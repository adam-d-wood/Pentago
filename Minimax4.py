...
def minimax(game, field, depth_remaining, maximisingPlayer):
    # print(maximisingPlayer)
    board = game.board
    if depth_remaining: print(depth_remaining)
    if wincheck(game, field) or depth_remaining <= 0:
        # print('done')
        if wincheck(game,field): print('end')
                    ...
                    for m in range(board.micro_width):
                        if field[i][j][k][m] == 0:
                            for n in range(8):
                                ...
                                # print('call')
                                # showfield(newfield)
                                minmax = minimax(game, newfield, depth_remaining-1, False)
                                try:
                                    if minmax >= value:
                                        value = minmax
                                        # print('max', value)
                    ...
                    for m in range(board.micro_width):
                        if field[i][j][k][m] == 0:
                            for n in range(8):
                                ...
                                newfield[tile[0]][tile[1]] = new_tile
                                # showfield(newfield)
                                minmax = minimax(game, newfield, depth_remaining-1, True)
                                try:
                                    if minmax <= value:
                                        value = minmax
                                        # print('min', value)
